...
var _url_loaded={};
var _metrics=null;
if(window.ue_t0!=null&&window.amznJQ.metrics==1){_metrics={tc:0,v:{}};
}this.startTimer=function(){if(_metrics==null){return;
};
if(_metrics.tc<1){;
_metrics.t=new Date().getTime();
}_metrics.tc++;
};
this.startTimer();
this.endTimer=function(){if(_metrics){if(_metrics.tc>0){;
_metrics.tc--;
if(_metrics.tc<1){if(_metrics.v["js"]==null){_metrics.v["js"]=0;
}_metrics.v["js"]+=new Date().getTime()-_metrics.t;
_metrics.t=null;
;
}}}};
this.recordTimer=function(name){if(_metrics){_metrics.v[name]=_metrics.v["js"];
;
}};
this.recordEvent=function(name){if(_metrics){_metrics.v[name]=new Date().getTime()-window.ue_t0;
;
}};
this.isMetricsEnabled=function(){return _metrics!=null;
};
function _sendMetrics(){if(_metrics){amznJQ.onReady("clientLog",function(){if(window.clientLogger){_metrics.v["appName"]="jQuery";
window.clientLogger.sendCLOGEntry("jq_logging","page_perf",_metrics.v);
_metrics.v={};
}});
}}var _loading={};
...
if(all_loaded){}};
}this.addLogical=function(functionality,urls){this.startTimer();
if(urls){;
}else{;
...
this.declareAvailable=function(functionality){this.startTimer();
;
...
this.available=function(functionality,eventCallbackFunction){this.startTimer();
...
if(amznJQ.isMetricsEnabled()){amznJQ.recordEvent(stage);
amznJQ.recordTimer(stage+".js");
}$(document).trigger(stage);
}};
...
}delete bootstrapAmznJQ;
if(amznJQ.isMetricsEnabled()){amznJQ.onReady("jQuery",function(){amznJQ.recordEvent("jq.r");
});
}amznJQ.endTimer();
}(jQuery));
...
var getPageHeight=function(){return $.browser.msie?$(document).height():"100%"
};
...
var marginLeft=parseInt(container.css("margin-left"))||0;
var marginRight=parseInt(container.css("margin-right"))||0;
...
}var NAV_POPOVER_SKIN='<table cellspacing="0" cellpadding="0" class="nav_popover">'+'<tr class="header"><td class="l navSpritePopoverV"></td><td class="c navSpritePopoverH"></td><td class="r navSpritePopoverH"></td></tr>'+'<tr class="body"><td class="l navSpritePopoverV"></td><td class="ap_content"></td><td class="r navSpritePopoverV"></td></tr>'+'<tr class="footer"><td class="l navSpritePopoverH"></td><td class="c navSpritePopoverH"></td><td class="r navSpritePopoverV"></td></tr>'+"</table>";
...
var initYourListsButton=function(){if(!$("#navYourListsButton").length){return;
...
var initNavShopAllButton=function(){if(!$("#navShopAllButton").length){return;
...
}var onShow=function(popover,settings){$("<div id='nav_mask_"+this.attr("id")+"' class='nav_shop_all_mask'><div class='inner navSpritePopoverH'/></div>").css({top:this.offset().top,left:this.offset().left+this.outerWidth()+_navbar._saMaskHorizOffset,height:this.outerHeight()-2}).prependTo(document.body);
...
$("li.navSaMenuItem").each(function(){$("#"+this.id).amazonPopoverTrigger({localContent:"#navSaChild_"+this.id.split("_")[1],width:null,location:"right",locationAlign:"top",locationMargin:0,locationOffset:[_navbar._saFlyoutHorizOffset,0],hoverShowDelay:hoverDelay,hoverHideDelay:hoverDelay,showCloseButton:false,showOnHover:true,forceAlignment:true,focusOnShow:false,skin:NAV_POPOVER_SKIN,onShow:onShow,onHide:onHide});
});
};
...
}closures.push(function(){if(newCount>0||newCount=="99+"){if(newCount>99){newCount="99+";
}cartCount.html(newCount);
cartEmpty.css("display","none");
cartFull.css("display","");
}else{cartCount.html("");
...
//! $DateTime$
(function(window,undefined){var $=window.jQuery,merchRE=/^me=/,refre=/(ref=[-\w]+)/,ddre=/_dd_/,ddaliasre=/(dd_[a-z]{3,4})(_|$)[\w]*/,deptre=/\{department\}/g,slashre=/\+/g,aliasre=/search-alias\s*=\s*([\w-]+)/,dcs="#ddCrtSel",sdpc="searchDropdown_pop_conn",tostr=Object.prototype.toString,ddBox;$.isArray=$.isArray||function(o){return tostr.call(o)==="[object Array]";};var SS=function(sb){var node
,div;function move(){node.css({width:sb.size().width,left:sb.pos().left});}
function create(){var p=sb.pos(),d=sb.size();$(window).resize(move);node=$("<div id='srch_sggst'/>").css({"background-color":"#FFF",border:"1px solid #5C8295",color:"#000",display:"none",left:p.left,position:"absolute","text-align":"left",top:p.top+d.height,width:d.width,"z-index":250}).appendTo(sb.parent());return div=node.get(0);}
function get(){return div||create();}
this.visible=function(){return get().style.display!="none";};this.hide=function(){get().style.display="none";this.html('');};this.show=function(){get().style.display="";};this.html=function(h){get().innerHTML=h;};this.content=function(c){this.html('');node.append(c).show();};};var SB=function(sb,h){var curText
,hasFocus
,curSel;init();function init(){sb.keydown(keyDown)
.keyup(keyUp)
.keypress(keyPress)
.blur(blurHandler)
.focus(focusHandler);curText=kw();}
function kw(k){if(k!==undefined){curText=curSel=k;}
return sb.val(k);}
function keyDown(e){var key=e.keyCode,d=key==38?-1:key==40?1:0;if(d){h.adjust(d);e.stopPropagation();return false;}
}
function keyUp(e){var key=e.keyCode;switch(key){case 13:h.hide();break;case 27:return h.dismiss();case 37:case 38:case 39:case 40:break;default:update(true);break;}}
function keyPress(e){var key=e.keyCode;switch(key){case 13:return h.submitEnabled();default:h.keyPress();break;}}
function update(dontCheckCurSel){var val=kw();if(val!=curText&&(dontCheckCurSel||val!=curSel)){h.change(curText=val);}}
function focusHandler(e){hasFocus=true;}
function blurHandler(e){h.dismiss();hasFocus=false;}
this.keyword=function(k){return kw(k);};this.size=function(){return{width:sb.outerWidth(),height:sb.outerHeight()};};this.pos=function(){return sb.position();};this.parent=function(){return sb.parent();};this.hasFocus=function(){return hasFocus;};this.update=update;this.blur=function(e){sb.blur(e);};this.focus=function(e){sb.focus(e);};};var KL=function(sb,f){var lastKeyPressTime,keyPressTimes=[],firstSuggestions=[],fsIdx=-1,binCount=10;init();function init(){sb.focus(focus);f.submit(formOnSubmit);}
function formOnSubmit(){updateKeyPressTimes();updateKeywordLogging(sb.keyword());}
function focus(){lastKeyPressTime=now();}
function buildInteractionHistogram(){if(!keyPressTimes||keyPressTimes.length<1){return null;}
var kptMin=Math.min.apply(Math,keyPressTimes),kptMax=Math.max.apply(Math,keyPressTimes),binSize=Math.ceil((kptMax-kptMin+1)/binCount),interactionHistogram=bzero(binCount,0),i,len,currentBin;for(i=0,len=keyPressTimes.length;i<len;i++){currentBin=Math.floor((keyPressTimes[i]-kptMin)/binSize);interactionHistogram[currentBin]++;}
return interactionHistogram.join('_')+'.'+kptMin.toFixed(0)+'_'+
binSize.toFixed(0);}
function updateKeywordLogging(searchBoxValue){if(searchBoxValue){var ih=buildInteractionHistogram();ih&&input(f,"ihelem","ih",ih);var firstCorrect=arrayIndexOf(firstSuggestions,searchBoxValue);firstCorrect&&input(f,"fscelem","fsc",firstCorrect);}}
function updateKeyPressTimes(){var t=now();if(lastKeyPressTime!=null){keyPressTimes.push(t-lastKeyPressTime);firstSuggestions[keyPressTimes.length-1]=null;}
lastKeyPressTime=t;}
this.times=function(){updateKeyPressTimes();};this.prepare=function(){if(keyPressTimes.length){fsIdx=keyPressTimes.length-1;}};this.update=function(k){if(fsIdx>=0){firstSuggestions[fsIdx]=k;fsIdx=-1;}};};var AC=function(opts){var version=1,opt={},names,values
,crtSel=-1
,suggestionList=[]
,curSize=0
...
,timer=null
,maxSuggestions=10
,maxCategorySuggestions=4,suggestRequest=null
,first=-1
,defaultDropDownVal
,insertedDropDownVal
...
,searchSuggest
,requestedKeyword=""
,activityAllowed=true,defaults={sb:"#twotabsearchtextbox",form:"#navbar form[name='site-search']",dd:"#searchDropdownBox",cid:"amazon-search-ui",action:"",sugPrefix:"issDiv",sugText:"Search suggestions"};opts&&init(opts);function init(opts){$.extend(opt,defaults,opts);var src=opt.src;staticContent=$.isArray(src);lookup(opt,"sb");if(!opt.sb){return;}
searchBox=new SB(opt.sb,{adjust:move,hide:hideSuggestions,dismiss:dismissSuggestions,change:(staticContent?update:delayUpdate),submitEnabled:submitEnabled,keyPress:keyPress});searchSuggest=new SS(searchBox);lookup(opt,"form");lookup(opt,"valInput");lookup(opt,"dd");lookup(opt,"submit");ddBox=opt.dd;opt.protocol=opt.protocol||window.parent.document.location.protocol;if(staticContent){names=src[0];values=src[1];opt.sb.removeAttr('style');}else{}
if(opt.submit){disable('disabled');opt.submitImgDef=opt.submit.attr("src");opt.submitToggle=opt.submitImgDef&&opt.submitImg;}
opt.keylog=opt.keylog?new KL(searchBox,opt.form):{prepare:nop,update:nop,times:nop};if(opt.ime){window.setInterval(function(){searchBox.update();},20);}
...
function initDynamic(sb,form,dd,service,mkt,aliases,handler,deptText,sugText,sc,dummy0,keylog,hilite){init({aliases:aliases,dd:dd,deptText:deptText,form:form,handler:handler,hilite:hilite,ime:(mkt==6||mkt==3240),keylog:keylog,mkt:mkt,sb:sb,sc:sc,src:service,sugText:sugText});}
function lookup(h,k,n){if(n=h[k]){n=$(n);if(n&&n.length){h[k]=n;return n;}}
delete h[k];}
function disable(d){opt.submit.attr('disabled',d);}
function move(n){if(curSize<=0){return;}
try{unhighlightCurrentSuggestion();if(n>0&&crtSel>=curSize-1){crtSel=-1;}else if(n<0&&crtSel<0){crtSel=curSize-1;}else{crtSel+=n;}
highlightCurrentSuggestion(true);}catch(e){}}
function hideSuggestions(){!opt.ime&&hideSuggestionsDiv();}
function dismissSuggestions(){if(searchSuggest.visible()){hideDelayTimerId=setTimeout(function(){return(function(){hideDelayTimerId=null;hideSuggestionsDiv();});}(),300);crtSel=-1;updateCrtSuggestion();return false;}else{return true;}}
function update(kw){suggestionList=[];if(!kw.length){hideSuggestionsDiv();hideNoMatches();}else{first=-1;if(opt.multiword){findSeq();}else{findBin();}
curSize=suggestionList.length;displaySuggestions(kw);checkForExactMatch();}
timer=null;crtSel=-1;}
function delayUpdate(kw){if(timer){clearTimeout(timer);timer=null;}
timer=setTimeout(function(){return(function(){opt.keylog.prepare();searchJSONSuggest();timer=null;crtSel=-1;});}(),100);}
function submitEnabled(){var s=opt.submit;return!s||!s.attr('disabled');}
function updateKeyPressTimes(){opt.keylog.times();}
function keyPress(key){opt.keylog.times(key);keystroke&&keystroke(key);}
...
function findBin(){var low=0,high=names.length-1,mid=-1,dataPrefix='',crtPrefix=keyword().toLowerCase(),len=crtPrefix.length;while(low<=high){mid=Math.floor((low+high)/2);dataPrefix=names[mid].substr(0,len).toLowerCase();if(dataPrefix<crtPrefix){low=mid+1;}else{high=mid-1;if(dataPrefix==crtPrefix){first=mid;}}}
if(first!=-1){var i=first,n;do{suggestionList.push({keyword:names[i],i:i});++i;}while((n=names[i])&&!n.toLowerCase().indexOf(crtPrefix));}}
function findSeq(){var crtPrefix=keyword().toLowerCase(),rexp=new RegExp("(^|(?:\\s))"+crtPrefix,"i"),i=0,len=names.length,n;for(;i<len;i++){n=names[i];if(n.match(rexp)){suggestionList.push({keyword:n,i:i});if(first==-1){first=i;}}}}
function checkForExactMatch(){var state='disabled';if(curSize){var sg=suggestionList[0].keyword,kw=keyword();if(sg.length==kw.length&&!getPrefixPos(sg,kw)){updateForm(first);state='';}}
disable(state);}
function displaySuggestions(crtPrefix){var sugDivId,lineText,line,sPrefix=opt.sugPrefix,prefix="#"+sPrefix,h='';searchSuggest.html('');if(curSize>0){hideNoMatches();searchSuggest.show();if(!staticContent){h+='<div id="sugdivhdr" align="right"> '+opt.sugText+'</div>';}}else{showNoMatches();}
for(i=0;i<curSize;i++){line=suggestionList[i];sugDivId=sPrefix+i;lineText=line.alias?getFormattedCategoryLine(line,crtPrefix):getFormattedSuggestionLine(line,crtPrefix);h+='<div id="'+sugDivId+'" class="suggest_link">'+lineText+'</div>';}
if(curSize>0){h+='<div id="sugdivhdr" align="right">&nbsp;</div>';}
h&&searchSuggest.html(h);if(ddBox){defaultDropDownVal=ddBox.val();}
for(i=0;i<curSize;++i){$(prefix+i).mouseover(suggestOver).mouseout(suggestOut).click(setSearchByIndex);}}
function showNoMatches(){if(opt.noMatch){var nmDiv=$('#'+opt.noMatch);searchSuggest.content(nmDiv.clone().attr('class','suggest_link suggest_nm').css({'display':'block'}));opt.submitToggle&&opt.submit.attr("src",opt.submitImg);}else{hideSuggestionsDiv();}}
function hideNoMatches(){if(opt.noMatch){$('#'+opt.noMatch).hide();opt.submitToggle&&opt.submit.attr("src",opt.submitImgDef);}}
function setSearchByIndex(){var divId=this.id;crtSel=parseInt(divId.substr(6));updateCrtSuggestion();searchSuggest.hide();if(!delayedDOMUpdate){opt.form.submit();}else{window.setTimeout(function(){opt.form.submit();},10);}}
function updateCrtSuggestion(){var val=keyword(),alias,categoryName,sg;if(crtSel>=0){sg=suggestionList[crtSel];val=sg.keyword;alias=sg.alias;categoryName=sg.categoryName;}
keyword(val);if(staticContent){if(crtSel>=0){updateForm(sg.i);disable('');}else{checkForExactMatch();}}else{updateCategoryDropDown(alias,categoryName);setDynamicSearch(crtSel,alias);}}
function setDynamicSearch(indexInList,alias){var prefixElems=$("#issprefix");if(indexInList>=0){var issMode,kw=keyword();if(alias){issMode='ss_c';}else if(opt.sc&&isSpellCorrection(suggestionList,indexInList)){issMode='ss_sc';}else{issMode='ss_i';}
setSearchFormReftag(opt.form,null,issMode,indexInList,kw.length);if(prefixElems.length){prefixElems.attr("value",kw);}else{input(opt.form,"issprefix","sprefix",kw);}}else{prefixElems.remove();}}
function suggestOver(event){this.style.cursor="default";unhighlightCurrentSuggestion();crtSel=parseInt(this.id.substr(6));highlightCurrentSuggestion(false);}
...
function highlightCurrentSuggestion(updateSearchBox){updateSearchBox&&updateCrtSuggestion();highlightSuggestion($('#'+opt.sugPrefix+crtSel));}
function unhighlightCurrentSuggestion(){unhighlightSuggestion($('#'+opt.sugPrefix+crtSel));}
function updateCategoryDropDown(alias,categoryName){var dd=ddBox,toRemove,val;if(!dd){return;}
val=alias?('search-alias='+alias):defaultDropDownVal;toRemove=(val==insertedDropDownVal||defaultDropDownVal==insertedDropDownVal)?null:insertedDropDownVal;if(alias){var sel=findOption(dd,val);insertedDropDownVal=null;if(!sel.length){dd.append(option(val,categoryName));insertedDropDownVal=val;}}
try{delayedDOMUpdate=false;$(dcs).length&&changeDropdownSelection(val,categoryName,true);dd.val(val);}catch(e){delayedDOMUpdate=true;}
...
,suppressFormatting=(start==-1&&!opt.hilite);if(!suppressFormatting){len=crtPrefix.length;if(!opt.hilite){kw=kw.substr(0,start)+bold(kw.substr(start,len))+kw.substr(start+len);}else if(curSuggestionLine.sc){kw=bold(kw);}else{kw=kw.substr(0,len)+bold(kw.substr(start+len));}}
return kw;}
function getFormattedCategoryLine(categoryLine,crtPrefix){return opt.deptText?getFormattedSuggestionLine(categoryLine,crtPrefix)
+' <span class="suggest_category">'
+opt.deptText.replace(deptre,categoryLine.categoryName)
+'<span>':categoryLine.categoryName;}
function hideSuggestionsDiv(){if(suggestRequest){suggestRequest.cleanup();suggestRequest=null;}
curSize=0;searchSuggest.hide();crtSel=-1;}
function updateForm(index){var v=values[index],fp;if(opt.valInput&&opt.valInput.length){opt.valInput.attr("value",v);}else{fp=getFormParams(v||location.href);cleanForm();populateForm(fp);}}
function getFormParams(url){var splitUrl=url.split("?"),query=splitUrl.length>1?splitUrl[1]:undefined,params=query?query.split("&"):[],i=params.length,pair;while(i-->0){pair=params[i].split("=");params[i]={name:pair[0],value:pair[1].replace(slashre,' ')};}
return{uri:splitUrl[0],formParams:params};}
function cleanForm(){opt.form.find(".frmDynamic").remove();}
function populateForm(formData){opt.form.attr("action",formData.uri);for(var i=0;i<formData.formParams.length;i++){var param=formData.formParams[i];input(opt.form,"frmDynamic",param.name,unescape(param.value),1);}}
function keyword(k){return searchBox.keyword(k);}
function searchAlias(alias){if(alias){changeDropdownSelection(alias);}else{var aliasName=ddBox.attr("value").match(aliasre);return aliasName?aliasName[1]:null;}}
function suggestions(){return suggestionList;}
function supportedSearchAlias(alias){var a=opt.aliases;return a&&arrayIndexOf(a,alias)>=0;}
function isSpellCorrection(info,index){return info[index].sc=="1";}
function combineSuggestions(crtSuggestions,extraData){var crtCategorySuggestions=extraData,combinedList=[],i=0,j=0,n=crtSuggestions.length,m,sg,cs;if(extraData&&extraData.length){crtCategorySuggestions=extraData[0].nodes;}
crtCategorySuggestions=crtCategorySuggestions||[];m=crtCategorySuggestions.length;while(i+j<maxSuggestions&&i<n){sg={keyword:crtSuggestions[i],sc:isSpellCorrection(extraData,i)};combinedList.push(sg);if(!i&&m>0){while(j<m&&j<maxCategorySuggestions){cs=crtCategorySuggestions[j];sg={keyword:crtSuggestions[i],sc:isSpellCorrection(extraData,i),alias:cs.alias,categoryName:cs.name};combinedList.push(sg);++j;}}
++i;}
curSize=combinedList.length;return combinedList;}
function searchJSONSuggest(){suggestRequest&&suggestRequest.cleanup();if(!activityAllowed){return;}
if(!searchBox.hasFocus()){return;}
var alias=searchAlias(),kw=keyword();if(!supportedSearchAlias(alias)){return;}
if(!kw||!kw.length){hideSuggestionsDiv();return;}
var str=encodeURIComponent(kw),suggestUrl=opt.protocol+'//'+opt.src+'?'+'method=completion'+
'&q='+str+
'&search-alias='+alias+
'&client='+opt.cid+
'&mkt='+opt.mkt+
'&x=updateISSCompletion';if(opt.sc){suggestUrl+='&sc=1';}
requestedKeyword=kw;suggestRequest=new A9JSONClient();suggestRequest.callSuggestionsService(suggestUrl);}
function updateCompletion(){if(!suggestRequest){return;}
suggestRequest.cleanup();suggestRequest=null;if(!activityAllowed||!completion.length||!completion[0]||!requestedKeyword||completion[0].toLowerCase()!=requestedKeyword.toLowerCase()){requestedKeyword="";return;}
if(!searchBox.hasFocus()){return;}
suggestionList=combineSuggestions(completion[1],(completion.length>2)?completion[2]:[]);opt.keylog.update(curSize?suggestionList[0].keyword:undefined);displaySuggestions(completion[0]);sugHandler&&sugHandler(completion[0],suggestionList);}
function stop(){activityAllowed=false;requestedKeyword="";if(suggestRequest){suggestRequest.cleanup();suggestRequest=null;}}
function start(){activityAllowed=true;}
function encoding(){var encInput=opt.form.find("input[name^='__mk_']");if(encInput.length){return[encInput.attr("name"),encInput.val()];}}
function blur(){searchBox.blur();}
return{suggest:bindSuggest,keypress:bindKeypress,submit:bindSubmit,blur:blur,keyword:keyword,searchAlias:searchAlias,stop:stop,start:start,encoding:encoding,initStaticSuggestions:initStatic,initDynamicSuggestions:initDynamic,updateAutoCompletion:updateCompletion,init:nop};};function now(){return(new Date).getTime();}
function nop(){}
function suppress(){return false;}
function bzero(len,val){var a=[];while(len--){a.push(val);}
return a;}
function arrayIndexOf(a,v){for(var i=0,len=a.length;i<len;i++){if(a[i]==v){return i;}}
return-1;}
...
function getShortenedIDForOption(o){var eq;if(!o||!o.length||(eq=o.indexOf("="))==-1){return'';}
var alias=o.substr(eq+1),dash=alias.indexOf("-")+1,shortID=alias.substr(0,3);return dash?shortID:(shortID+alias.charAt(dash));}
function changeDropdownSelection(optionValue,selectedDisplayName,highlightOnly,option){var dd=ddBox;if(optionValue=='search-alias=aps'&&!selectedDisplayName){selectedDisplayName=findOption(dd,optionValue).text();}
$('#'+sdpc).css("visibility","hidden");$(dcs).text(selectedDisplayName);dd.val(optionValue);if(!highlightOnly){opt.sb.focus();setSearchFormReftag(opt.form,optionValue);}}
function setSearchFormReftag(formElement,optionValue,issMode,indexInList,numUserChars){var formAction=formElement.attr('action'),isstag=(issMode!=null&&indexInList!=null),tag=isstag?issMode+'_'+indexInList+'_'+numUserChars:'dd_'+getShortenedIDForOption(optionValue);if(isstag||optionValue!=null){if(!refre.test(formAction)){if(formAction.charAt(formAction.length-1)!='/'){formAction+='/';}
formAction+=tag;}else if(isstag&&ddaliasre.test(formAction)){formAction=formAction.replace(ddaliasre,'$1_'+tag);}else{formAction=formAction.replace(refre,'ref=nb_sb_'+tag);}
formElement.attr("action",formAction);}}
function mergeDropDownEntries(btf){var i=0,dd=ddBox,crt,crtVal,btfVal,btfText,btflen=btf.length,kids=dd.children(),len=kids.length,j=len,vals=[];while(j-->0){var crt=$(kids[j]).attr("value");if(crt.match(merchRE)){return;}
vals[j]=crt;}
len=Math.min(len,btflen);for(j=0;j<len;j++){crtVal=vals[j];if(!i&&crtVal!=btf[0][0]){continue;}
crt=$(kids[j]);while(i<btflen&&(btfText=btf[i][1])&&crtVal!=(btfVal=btf[i++][0])){option(btfVal,btfText).insertBefore(crt);}}
while(i<btflen&&(btfText=btf[i][1])&&(btfVal=btf[i++][0])){dd.append(option(btfVal,btfText));}}
function A9JSONClient(){var fullUrl,noCacheIE,headLoc,scriptId,scriptObj,scriptCounter=1;function callService(url){fullUrl=url;noCacheIE='&noCacheIE='+now();headLoc=document.getElementsByTagName("head").item(0);scriptId='JscriptId'+scriptCounter++;buildScriptTag();addScriptTag();}
function buildScriptTag(){scriptObj=document.createElement("script");scriptObj.setAttribute("type","text/javascript");scriptObj.setAttribute("charset","utf-8");scriptObj.setAttribute("src",fullUrl+noCacheIE);scriptObj.setAttribute("id",scriptId);}
function removeScriptTag(){try{headLoc.removeChild(scriptObj);}catch(e){}}
function addScriptTag(){headLoc.appendChild(scriptObj);}
return{callSuggestionsService:callService,cleanup:removeScriptTag};};function searchDropdown(){var popoverLoaded=false,popoverWidth=410,connElem,popoverTab=$('#searchDropdownContent_tab'),popoverOpen=false;function initConnDiv(){connElem=$("<div id='"+sdpc+"' class='dropdownConnDiv'></div>");$('body').append(connElem);}
function createDropdownItem(option,tabIdx){var displayText=option.text(),optionValue=option.attr("value"),selected=option.attr("selected"),current=option.attr("current"),container=$("<label>"),item=$("<div class='searchDropdownOption ap_custom_close'>"+displayText+"</div>").appendTo("body"),clazz="searchDropdownOptionHover";selected&&item.addClass('selectedDropdownOption');if(current=='parent'){item.css('font-weight','bold');}
item.hover(function(){item.addClass(clazz);},function(){item.removeClass(clazz);}).click(function(){item.removeClass(clazz);changeDropdownSelection(optionValue,displayText,false,option);}).keyup(function(e){var w=e.which;if(w==9){changeDropdownSelection(optionValue,displayText,'true',option);}else if(keyClose(w)){item.click();}
return false;});return tabIndex(item,tabIdx);}
function createDropdownContents(){var ddOptions=ddBox.find('option'),numOfOptions=ddOptions.length,curOption=ddOptions.filter("option[current=true]").length
,numOfGeneralOptions=curOption?numOfOptions-1:numOfOptions,tableWidthOffset=70,twoColumn;if(numOfGeneralOptions<14){popoverWidth=popoverTab.width()+28;tableWidthOffset=tableWidthOffset/2+33;}else{twoColumn=Math.ceil(numOfGeneralOptions/2);}
var tableWidth=popoverWidth-tableWidthOffset,currentOptionTable=$("<table width='"+tableWidth+"'>"),currentOptionRow=$("<tr/>").appendTo(currentOptionTable),currentOptionSpot=$("<td/>").appendTo(currentOptionRow),id="searchPopoverContents",tableDiv=$("<div id='"+id+"' style='display: none'>").appendTo(currentOptionTable)
,dropTable=$("<table>").css({width:tableWidth,position:"relative","margin-top":-20})
,tabIdx=1,rowArray;if(twoColumn){rowArray=[];rowArray.length=twoColumn;}
ddOptions.each(function(i,v){tabIdx++;var dropRow,t=$(v);if(t.attr("current")=='true'){var currentOption=createDropdownItem(t,tabIdx).appendTo(currentOptionSpot);currentOptionTable.addClass('currentDropdownOption');dropTable.css("margin-top",0);return true;}
if(twoColumn&&i>=twoColumn){dropRow=rowArray[i-twoColumn];}else{dropRow=$("<tr>").appendTo(dropTable);if(twoColumn){rowArray[i]=dropRow;}}
createDropdownItem(t,tabIdx).appendTo($("<td width='50%'>").appendTo(dropRow));});dropTable.appendTo(tableDiv.appendTo('body'));return $('#'+id);};function updateOptionDisplayName(){var dd=ddBox,ddSelectionName=dd.find('option:selected').text(),displayTab=$(dcs);if(displayTab.text()!=ddSelectionName){var reverseOption=dd.find('option[text="'+displayTab.text()+'"]');dd.val(reverseOption.val());}}
function popoverClick(){if(popoverOpen){popoverTab.click();}}
amznJQ.onReady('popover',function(){updateOptionDisplayName();tabIndex(popoverTab,"0");popoverTab.keyup(function(e){if(keyClose(e.which)){popoverTab.click();$('#site-search').submit(suppress);}
return false;});var sb=$('#twotabsearchtextbox');sb.focus(popoverClick).blur(popoverClick);initConnDiv();popoverTab.amazonPopoverTrigger({showOnHover:false,showCloseButton:false,forceAlignment:true,localContent:createDropdownContents(),location:"bottom",width:popoverWidth,zIndex:200,skin:"default",locationMargin:4,locationOffset:[1,-2],closeEventInclude:"CLICK_OUTSIDE",onShow:function(popover){tabIndex(popoverTab,"1");popoverTab.focus();var pos=popoverTab.offset();connElem.css({left:pos.left+1,top:pos.top+16,visibility:"visible",width:popoverTab.width()-5});popoverOpen=true;$('.ap_content').css("padding",0);return 1;},onHide:function(popover){tabIndex(popoverTab,"0");connElem.css("visibility","hidden");popoverOpen=false;return 1;}});});};var btfdd=[["aps","All Departments",'137'],["automotive","Automotive",'1'],["baby","Baby",'3'],["baby-products","Baby",'13'],["beauty","Beauty",'13'],["stripbooks","Books",'137'],["automotive","Car & Motorbike",'3'],["wireless-aps","Cell Phones & Service",'1'],["classical","Classical Music",'37'],["clothing","Clothing",'3'],["apparel","Clothing & Accessories",'1'],["diy","DIY & Tools",'3'],["dvd","DVD",'37'],["electronics","Electronics",'17'],["electronics","Electronics & Photo",'3'],["outdoor","Garden & Outdoors",'3'],["grocery","Grocery & Gourmet Food",'1'],["drugstore","Health & Beauty",'3'],["hpc","Health & Personal Care",'1'],["garden","Home & Garden",'1'],["kitchen","Home & Garden",'7'],["industrial","Industrial & Scientific",'1'],["jewelry","Jewellery",'3'],["jewelry","Jewelry",'1'],["digital-text","Kindle Store",'1'],["kitchen","Kitchen & Home",'3'],["lighting","Lighting",'3'],["magazines","Magazine Subscriptions",'1'],["dvd","Movies & TV",'1'],["digital-music","MP3 Downloads",'13'],["popular","Music",'137'],["mi","Musical Instruments",'1'],["office-products","Office Products",'3'],["office-products","Office Products & Supplies",'1'],["shoes","Shoes",'1'],["shoes","Shoes & Accessories",'3'],["software","Software",'137'],["sporting","Sports & Outdoors",'1'],["sports","Sports & Leisure",'3'],["tools","Tools & Home Improvement",'1'],["toys","Toys & Games",'3'],["toys-and-games","Toys & Games",'1'],["vhs","VHS",'137'],["videogames","Video Games",'137'],["amazontv","Video On Demand",'1'],["watches","Watches",'13']];window.mergeBTFDropdown=function(){var btf=[],i=0,e,m=issMktid,len=btfdd.length;for(;i<len;i++){e=btfdd[i];if(e[2].indexOf(m)>=0){btf.push(["search-alias="+e[0],e[1]]);}}
mergeDropDownEntries(btf);};window.searchDropdown=searchDropdown;window.AutoComplete=AC;})(window);amznJQ.declareAvailable('search-js-autocomplete');
