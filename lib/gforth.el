...
(setq forth-words
	...
	(("[if]" "[unless]" "[?do]" "[do]" "[for]" "[begin]" 
	  "[endif]" "[then]" "[loop]" "[+loop]" "[next]" "[until]" "[repeat]"
	  "[again]" "[while]" "[else]")
	 immediate (font-lock-keyword-face . 2))
	(("[ifdef]" "[ifundef]") immediate (font-lock-keyword-face . 2)
	 "[ \t\n]" t name (font-lock-function-name-face . 3))
	(("if" "unless" "begin" "ahead" "do" "?do" "+do" "u+do" "-do" "u-do" "for" 
	  "case" "of" "otherwise" "?dup-if" "?dup-0=-if" "then" "endif" "until"
...
(setq forth-indent-words
      '((("if" "unless" "begin" "do" "?do" "+do" "-do" "u+do"
