...
;;  Functions to work with changesets
;;  There are some terminology problems.
;; darcsum-change-item takes a change (which I call cschange), and calls the LINE
;; in the description above ITEM. In the description above change is the cdr of the 
;; (line change) which I call change-line. Not what darcsum-change-item gets. 
;; Varibles for change-lines are usually called change.
;; Need to update the terminology or  documentation.

(defun csdir-name(csdir)
  "changeset directory name"
  (car csdir))

(defun csdir-files(csdir) 
  (cdr csdir))

(defun csfile-name(csfile)
  (car csfile))

(defun csfile-changes(csfile)
  (cdr csfile))

(defun cschange-line(cschange)
  (car cschange))

(defun cschange-changes(cschange)
  (cdr cschange))

(defun line-marked-p (line)
  (listp line))

(defun unmark-line (change-line)
  (setq change-line (car change-line)))

(defun cschange-marked-p(cschange)
  (line-marked-p (cschange-line cschange)))

(defun toggle-cschange(change)
  (if (cschange-marked-p change)
...
(defun darcsum-ensure-visible-item (item)
   (if (darcsum-item-visible-p item)
       item
       (darcsum-visible-item item)))
...
(defun darcsum-applicable-p (data predicate)
  (catch 'exit
    (ignore
     (let (dir file change)
       (dolist (dir data)
	 (dolist (file (csdir-files dir))
	   (dolist (change (csfile-changes file))
	     (if (funcall predicate (csdir-name dir) (csfile-name file) change)
		 (throw 'exit t)))))))))

(defsubst darcsum-marked-p (data)
  (darcsum-applicable-p data (function
			      (lambda (dir file change)
				(cschange-marked-p change)))))

(defun darcsum-add-directory-p (candidate-base-dir candidate-new-dir compare-dir)
  "Compare-dir is a selected patch in the changeset. Does this patch need 
adddir on candidate-new-dir first?"
  (let ((candidate (concat candidate-base-dir "/" candidate-new-dir)))
    (and (>= (length compare-dir) (length candidate))
	 (string= (substring compare-dir 0 (length candidate))
		  candidate))))

(defsubst darcsum-changeset-has-change-p (data odir ofile kind start-line replace)
  (dribble "---\\")
  (dribble odir)
  (dribble ofile)
  (dribble kind)
  ...
  (darcsum-applicable-p
   data (function
	 (lambda (d f change)
	  (if (eq 'adddir kind)
	      (darcsum-add-directory-p odir ofile d)
	      (and (equal odir d)
		   (equal ofile f)
		   (or (and (eq kind 'hunk)
			    (eq start-line (darcsum-ensure-visible-item (darcsum-change-item change))))
		       (and (not (eq kind 'hunk))
                            (eq kind (darcsum-ensure-visible-item (darcsum-change-item change)))))
		   (or (not (eq kind 'replace))
	  ...
	  (if (funcall predicate (csdir-name dir) (csfile-name file) change)
	      (setq changeset
		    (darcsum-add-changeset
		     changeset
		     (list (list (csdir-name dir) (list (csfile-name file) change)))))))))
    changeset))
...
(defun darcsum-apply-to-changeset (data func)
  (let (dir file change)
    (dolist (dir data)
      (dolist (file (csdir-files dir))
	(dolist (change (csfile-changes file))
	  (funcall func (csdir-name dir) (csfile-name file) change))))))
...
(defun darcsum-remove-changeset (data changeset)
  ...
  (let (dir file change)
    (dolist (dir changeset)
      (dolist (file (csdir-files dir))
	(dolist (change (csfile-changes file))
	  (let* ((dentry (assoc (csdir-name dir) data))
		 (fentry (assoc (csfile-name file) (cdr dentry))))
...
(defun darcsum-add-changeset (data changeset)
  ...
  (let (dir file change)
	...
	(dolist (change (csfile-changes file))
	  (let ((dentry (assoc (csdir-name dir) data)))
	    (if dentry
		(let ((fentry (assoc (csfile-name file) dentry)))
		  (if fentry
		    ...
		    (nconc dentry (list (list (csfile-name file) change)))))
	      (setq data (cons (list (csdir-name dir)
				     (list (csfile-name file) change))
			       data))))))))
  data)
...
(defun darcsum-merge-changeset (data changeset)
  ...
  (let (dir file change final-data)
    ...
    (dolist (dir data)
      (dolist (file (csdir-files dir))
	(dolist (change (csfile-changes file))
	  (let* ((dentry (assoc (csdir-name dir) changeset))
		 (fentry (assoc (csfile-name file) dentry))
		 (item (darcsum-change-item change))
	    ...
	    (when (and dentry fentry
			   ...
			   (assoc (darcsum-toggle-item item) (cdr fentry))
			   (assoc (list item) (cdr fentry))))
	      (unless (setq final-dentry (assoc (csdir-name dir) final-data))
		(setq final-data (cons (list (csdir-name dir)) final-data)
		      final-dentry (assoc (csdir-name dir) final-data)))
	      (unless (setq final-fentry (assoc (csfile-name file) final-dentry))
		(nconc final-dentry (list (list (csfile-name file))))
		(setq final-fentry (assoc (csfile-name file) final-dentry)))
  ...
  (let (dir file change line beg)
    (dolist (dir data)
      (insert
       (darcsum-add-props
	(concat "in directory "
		(darcsum-add-face (concat (csdir-name dir))
				  'darcsum-header-face t)
		":\n")
	'darcsum-line-type 'dir
	'darcsum-dir (csdir-name dir)))
      (dolist (file (csdir-files dir))
	(let* ((all-marked (listp (car (cadr file))))
	  ...
	  (insert
	   (darcsum-add-props
	    (concat "          "
		    ...
		    (darcsum-add-face (concat (csfile-name file))
				      'darcsum-filename-face t) "\n")
	    'darcsum-line-type 'file
	    'darcsum-dir (csdir-name dir)
	    'darcsum-file (csfile-name file)))
	  (dolist (change (if status nil (csfile-changes file)))
	  (let ((item (darcsum-change-item change)))
		...
		(insert (if (not (cschange-marked-p change))
			    line
  ...
  (with-current-buffer (process-buffer proc)
	...
	(let* ((kind (intern (match-string 1)))
	  ...
	  (when (looking-at
	    ...
	    ;;(when (eq kind 'hunk) (setq kind (string-to-number start-line)))
	    (let ((end (match-end 0))
		  (record (darcsum-changeset-has-change-p
			   darcsum-changeset-to-record
			   dir base kind
                           (when (eq kind 'hunk) (string-to-number start-line))
                           replace)))
...
(defun dribble(txt)
  (with-current-buffer (get-buffer-create "*darcsum dribble*")
        (goto-char (point-max))
        (insert (format "%s" txt))
        (newline)))
...
VIEW non-nil means open in View mode."
  ...
  (let* ((type (get-text-property (point) 'darcsum-line-type))
	...
	(with-current-buffer (cond ((eq other 'dont-select)
				   ...
				   (other (find-file-other-window file))
				   (t (find-file file)))
	  (when (line-marked-p change-line)
	        (unmark-line change-line))
  ...
  (dolist (dir (darcsum-selected-changeset))
	...
	(if (and (symbolp item) (eq item 'newfile))
	    (progn
	      ...
	      (with-temp-buffer
		(cd (expand-file-name (csdir-name dir)))
		(if (/= 0 (call-process darcsum-program nil t nil
					"add" (csfile-name file)))
		    (error "Error running `darcsum add' for `%s' in dir `%s'"
			   (csfile-name file) (csdir-name dir)))))
